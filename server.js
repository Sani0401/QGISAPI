...
app.get("/adjData", async (req, res) => {
  ...
  if (!state || !district || !tehsil || !village || !lgd_code || !khasra_no || !saltKey) {
    res.status(400).json({ message: "Invalid request, missing field." });
  }
...
try{
    ...
    async function fun() {
      ...
      if (!stateExists) {
        ...
        return res.status(400).json({ message: `wrong State :  ${d}` });
      }
    ...
    //tehsil check 
    async function fun2() {
      const stateExists = await checkStateExists2(tehsil);
      let d = tehsil;
      if (!stateExists) {
        console.log('Failure: tahshil does not exist in the Database.');
        await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
        return res.status(400).json({ message: `wrong tehsil : ${d} ` });
    ...
    async function fun3() {
      const stateExists = await checkStateExists3(lgd_code);
      let d = lgd_code;
      if (!stateExists) {
        console.log('Failure: lgd_code does not exist in the Database');
        await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
       return  res.status(400).json({ message: `wrong lgd_code : ${d} ` });
      }
    ...
    async function fun1(){
      const stateExists = await checkStateExists1(district);
      let d = district;
      if (!stateExists) {
        console.log('Failure: district does not exist in the Database');
        await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
        return res.status(400).json({ message: `wrong district : ${d} ` });
      }
    ...
    async function fun4(){
      const stateExists = await checkStateExists4(village);
      let d = village;
      if (!stateExists) {
        console.log('Failure: village does not exist in the Database');
        await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
        return res.status(400).json({ message: `wrong village ${d} ` });
      }
      console.log("village "+ stateExists);
      // Make sure to close the pool if necessary
      // pool.end();
    };
      ...
      const [resultFun, resultFun1, resultFun2, resultFun3, resultFun4] = await Promise.all([fun(), fun1(), fun2(), fun3(), fun4()]);

  // Now you can proceed with the rest of your code using the results
  
    return [resultFun && resultFun1 && resultFun2 && resultFun3 && resultFun4];
...
try{
  ...
  // console.log(queryResult.rows[0]); 
    ...
    if (queryResult.rows.length == 0) {
      ...
      res.status(404).json({ message: "Khasara not found" });
...
app.get("/adjData2", async (req, res) => {
...
//           console.log('Failure: State does not exist in the Database');
//           await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//           res.status(400).json({ message: `wrong state:  ${state}`});
//         }
//         console.log("state "+stateExists);
//         // Make sure to close the pool if necessary
//         // pool.end();
...
//           console.log('Failure: lgd_code does not exist in the Database');
//           await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//           res.status(400).json({ message: `wrong lgd_code : ${lgd_code}`},);
//         }
//         console.log("lgd_code "+stateExists);
//         // Make sure to close the pool if necessary
//         // pool.end();
...
//           console.log('Failure: district does not exist in the Database');
//           await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//           res.status(400).json({ message: `wrong district : ${district} ` });
//         }
//         console.log("district "+ stateExists);
//         // Make sure to close the pool if necessary
//         // pool.end();
...
//           console.log('Failure: village does not exist in the Database');
//           await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//           res.status(400).json({ message: `wrong village : ${village}` });
//         }
//         console.log("village "+ stateExists);
//         // Make sure to close the pool if necessary
//         // pool.end();
...
//                 await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//                 res.status(404).json({ message: "Khasara_no not found ",});
...
//                 await sendEmail(state, district, tehsil, village, lgd_code, khasra_no); 
//                 res.status(404).json({ message: "Field Geometry not found"})
//                }
...
//                 await sendEmail(state, district, tehsil, village, lgd_code, khasra_no);
//                 res.status(404).json({ message: "Field area_ac not found"})
//                }
...
//               }
//           } catch (error) {
//               // Handle the error when API access limit is exceeded
//               res.status(429).json({ message: 'API access limit exceeded' });
//           }
